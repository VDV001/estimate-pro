    ...
    <section className={cn("relative w-full mt-0 overflow-hidden", className)}>
      <footer className="border-t bg-background mt-20 relative">
        ...
        <div
          ...
          className={cn(
            "absolute bottom-24 md:bottom-20 backdrop-blur-sm rounded-3xl bg-background/60 left-1/2 border-2 border-border hover:border-foreground transition-colors flex items-center justify-center p-3 -translate-x-1/2 z-10",
            onBrandClick && "cursor-pointer"
          )}
        >
          <div className="w-12 sm:w-16 md:w-24 h-12 sm:h-16 md:h-24 bg-primary rounded-2xl flex items-center justify-center shadow-lg">
            {brandIcon || (
              <NotepadTextDashed className="w-8 sm:w-10 md:w-14 h-8 sm:h-10 md:h-14 text-primary-foreground drop-shadow-lg" />
            )}
          </div>
        </div>
